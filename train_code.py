...
a=np.load('wordHasVector.npy',allow_pickle=True)
wordHasVector=a.tolist()


twitter = pd.read_csv('twitter用户post文件.csv',encoding='utf8')
twitter.drop(axis=1, columns='Unnamed: 0',inplace=True)
user_post = []
all_user = []
embedding_sum = 0
for i in range(len(twitter['userid'])-1):
    for j in twitter['0'][i].replace("'","").replace("[","").replace(']','').split(','):
        try:
    ...
    post_embedding = embedding_sum/len(twitter['0'][i])
    user_post.append(post_embedding)
    if twitter['userid'][i+1] != twitter['userid'][i]:
...
x_normal = x_normal.reshape((1167,452,100))
