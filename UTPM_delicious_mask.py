...
def train():
    input_feature = torch.load('MVKE_user_post_delicious.pt')
    ...
    for epoch in range(end_epoch):
        ...
        f1_micro = f1_micro / 6
        f1_macro = f1_macro / 6
        f1_example = f1_example / 6

        writer.add_scalar('f1_score_micro_test_UTPM_delicious_post', f1_micro, epoch)
        writer.add_scalar('f1_score_macro_test_UTPM_delicious_post', f1_macro, epoch)
        writer.add_scalar('f1_example_test_UTPM_delicious_post', f1_example, epoch)

        writer.add_scalar('loss_train_UTPM_delicious_post', all_loss_train, epoch)
        writer.add_scalar('loss_test_UTPM_delicious_post', all_loss_test, epoch)
        print("Epoch {} training loss:".format(epoch), all_loss_train)
        print("Epoch {} testing loss:".format(epoch), all_loss_test)
    print("\n*****  Training Done  *****")
    torch.save(model.state_dict(), 'UTPM_delicious_post.pkl')
