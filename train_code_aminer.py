...
lengthVec = np.load('length_aminer.npy')

def train():
    # Create toy data
    # inputShape = np.shape(x_normal)
    #inputShape = [2,len(test[0]),100]
    inputData = torch.load('author_post_padding_aminer.pt')
    ...
    for epoch in range(end_epoch):
        ...
        scheduler.step()
        if epoch == 200:
                np.save('h_values_aminer_200.npy', h_value)
        if epoch == 300:
                np.save('h_values_aminer_300.npy', h_value)
        if epoch == 400:
                np.save('h_values_aminer_400.npy', h_value)
        if epoch == 500:
                np.save('h_values_aminer_500.npy', h_value)
        all_loss = all_loss/54

        writer.add_scalar('loss_amienr',all_loss , epoch)
