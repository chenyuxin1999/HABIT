    ...
    def forward(self, x):
        ...
        for i in range(x.shape[1] - 1):
            for j in range(i + 1, x.shape[1]):
                ...
                batch_xi_xj = x[:, i] * x[:, j]
                batch_c.append((batch_xi_xj * vi_vj))
        batch_c = torch.tensor([item.cpu().detach().numpy() for item in batch_c]).cuda()
...
def train():
    ...
    keyword_embedding = torch.load('keyword_embedding_merge.pt')
    ...
    dataTrain = DataLoader(dataset=train_data, batch_size=200, shuffle=True)
    dataTest = DataLoader(dataset=eval_data, batch_size=200, shuffle=True)
    ...
    for epoch in range(end_epoch):
        ...
        all_loss_train = all_loss_train / 44
        scheduler.step()
        ...
        all_loss_test = all_loss_test / 11
        f1_micro = f1_micro / 11
        f1_macro = f1_macro / 11
        f1_example = f1_example / 11
        rankingloss = rankingloss / 11
        writer.add_scalar('f1_score_micro_UTPM_merge', f1_micro, epoch)
        writer.add_scalar('f1_score_macro_UTPM_merge', f1_macro, epoch)
        writer.add_scalar('f1_example_UTPM_merge', f1_example, epoch)
        writer.add_scalar('loss_test_UTPM_merge', all_loss_test, epoch)
        writer.add_scalar('ranking_loss_UTPM_merge', rankingloss, epoch)
        print("Epoch {} testing loss:".format(epoch), all_loss_test)

        writer.add_scalar('loss_train_UTPM_merge', all_loss_train, epoch)
        print("Epoch {} training loss:".format(epoch), all_loss_train)
    print("\n*****  Training Done  *****")
    torch.save(model.state_dict(), 'UTPM_merge.pkl')
