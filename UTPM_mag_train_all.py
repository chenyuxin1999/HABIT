...
def train():
    ...
    keyword_embedding = torch.load('keyword_embedding_aminer.pt')
    ...
    for epoch in range(end_epoch):
        ...
        writer.add_scalar('f1_score_micro_UTPM_aminer', f1_micro, epoch)
        writer.add_scalar('f1_score_macro_UTPM_aminer', f1_macro, epoch)
        writer.add_scalar('f1_example_UTPM_aminer', f1_example, epoch)
        writer.add_scalar('loss_test_UTPM_aminer', all_loss_test, epoch)
        writer.add_scalar('ranking_loss_UTPM_aminer', rankingloss, epoch)
        print("Epoch {} testing loss:".format(epoch), all_loss_test)

        writer.add_scalar('loss_train_UTPM_aminer', all_loss_train, epoch)
        print("Epoch {} training loss:".format(epoch), all_loss_train)
    print("\n*****  Training Done  *****")
    torch.save(model.state_dict(), 'UTPM_aminer.pkl')
